...
// No remote fallback — callers must pass a local image.
const DEFAULT_IMAGE = '';
...
/**
 * Draggable peelable sticker (WebGL mesh).
 */
...
export default function StickerDrag(__props: StickerDragProps) {
  ...
  const parsedStaticShadow = useMemo(
    () => parseBoxShadow(staticShadow),
    [staticShadow],
  );
  const parsedDynamicShadow = useMemo(
...
const COMPONENT_DEFAULTS = {
  image: undefined as ResponsiveImageSource | undefined,
  imageWidth: 600,
  imageHeight: 400,
  tilt: 45,
  lighting: true,
